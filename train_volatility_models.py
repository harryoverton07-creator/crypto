...
def train_coin_volatility(coin):
    print(f"\n=== Training {coin} volatility model ===")
    pair = PAIR_MAP[coin]
    ...
    print(f"{coin} volatility accuracy: {acc:.3f}")

    filename = f"{coin.lower()}_model_volatility.pkl"
    joblib.dump(model, filename)
    print(f"Saved {filename}")

def main():
    for coin in ["BTC", "ETH", "SOL", "XRP", "ADA"]:
        train_coin_volatility(coin)
