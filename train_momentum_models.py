...
def label_momentum(candles):
    closes = np.array([c["close"] for c in candles])
    future = closes[-1] - closes[-3]  # momentum window
...
def build_dataset(coin):
    ...
    for i in range(100, len(candles)):
        ...
        y.append(label_momentum(window))

    return np.array(X), np.array(y)

for coin in ["BTC", "ETH", "SOL", "XRP", "ADA"]:
    print(f"\n=== Training {coin} momentum model ===")
    ...
    print(f"{coin} momentum accuracy:", model.score(X_test, y_test))
    joblib.dump(model, f"{coin.lower()}_model_momentum.pkl")
    print(f"Saved {coin.lower()}_model_momentum.pkl")
