...
def label_window(window):
    closes = np.array([c["close"] for c in window])
    ret_20 = (closes[-1] - closes[-20]) / closes[-20] * 100.0
    vol_40 = closes[-40:].std()
    price = closes[-1]
    baseline = price * 0.02

    if ret_20 > 3.0 and vol_40 <= baseline:
        return 0  # BREAKOUT_UP
    if ret_20 < -3.0 and vol_40 <= baseline:
        return 1  # BREAKOUT_DOWN
    if ret_20 > 1.0 and vol_40 > baseline * 1.5:
        return 2  # CHAOTIC_UP
    if ret_20 < -1.0 and vol_40 > baseline * 1.5:
        return 3  # CHAOTIC_DOWN
    return 4      # SIDEWAYS
...
def train_coin_regime(coin):
    print(f"\n=== Training {coin} regime model ===")
    pair = PAIR_MAP[coin]
    ...
    print(f"{coin} regime accuracy: {acc:.3f}")

    filename = f"{coin.lower()}_model_regime.pkl"
    joblib.dump(model, filename)
    print(f"Saved {filename}")

def main():
    for coin in ["BTC", "ETH", "SOL", "XRP", "ADA"]:
        train_coin_regime(coin)
